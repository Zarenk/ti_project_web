...
- ⚠️ **Paso 5 – TenantContext y servicios dependientes:** `TenantContextService` valida `allowedCompanyIds` y servicios clave (`stores`, `sales`, `clients`, `websales`, `entries`, `inventory`, `accounting`) ya aplican filtros por compañia/organización, registrando eventos con ambos identificadores. Reportes (`acc-reports`) incorporó controles al encolar/consultar jobs y cuenta con specs unitarios multi-tenant. Resta auditar endpoints secundarios de contabilidad y coordinar con los consumidores que lean el nuevo `companyId`.
...
1. **Backfill contable documentado:** el seed `npm run seed:populate-accounting-tenancy -- --dry-run --summary-path=tmp/accounting-tenancy.json` confirmó que 49 asientos ya poseen `organizationId`/`companyId`. Registrar la evidencia y programar correrlo sin `--dry-run` solo si aparecen nuevas filas nulas.
2. **Auditar lecturas contables restantes:** revisar endpoints secundarios y consultas Prisma que aún no usan `companyId` para cerrar la auditoría de contabilidad y dejar lista la fase de endurecimiento (`NOT NULL`/FK).
3. **Documentar ejecuciones de prueba:** capturar salidas de `npm test`, `npm run seed:populate-accounting-tenancy -- --dry-run ...` y `npm run test:e2e`, adjuntando fixtures multi-compañía y checklist de QA previo a exponer cambios en frontend.
