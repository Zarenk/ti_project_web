...
- `npm run ci:multi-tenant-report`: automatiza la corrida E2E con m?tricas multi-tenant; acepta `--phase3` y banderas `--phase3-*` para ejecutar `phase3:run` previo a las pruebas (p. ej. `--phase3-dry-run=false`, `--phase3-summary-dir=...`, `--phase3-print-options`, `--phase3-env PHASE3_DEFAULT_ORG_CODE=tenant-alpha`, `--phase3-env PHASE3_OPTIONS_PATH=tmp/phase3/options.json`).
- Workflow `Multi-tenant Coverage`: corre `npm run ci:multi-tenant-report -- --phase3`, genera el reporte consolidado con `npm run phase3:report -- --dir tmp/phase3 --output tmp/phase3/report.json` y adjunta los JSON (`populate-summary`, `validate-summary`, `options.json`, `report.json`) junto con los artefactos de m?tricas/badge.
- `npm run phase3:report`: genera un reporte consolidado (`totales`, entidades con actualizaciones, hallazgos de validaci?n) a partir de `populate-summary.json` y `validate-summary.json`, imprime el resultado en STDOUT (`--summary-stdout`) y permite guardarlo con `--output=<ruta>`. Tambi?n acepta rutas directas (`--populate`, `--validate`) o un directorio (`--dir`) para descubrir los res?menes generados por `phase3:run`, e interpreta autom?ticamente las rutas definidas en `PHASE3_REPORT_DIR`, `PHASE3_REPORT_POPULATE`, `PHASE3_REPORT_VALIDATE`, `PHASE3_REPORT_OUTPUT`, `PHASE3_OPTIONS_PATH`, `PHASE3_REPORT_STDOUT`.
- `npm run phase3:staging`: orquesta la corrida supervisada en staging ejecutando `phase3:run`, generando `populate-summary.json`, `validate-summary.json`, `options.json` y `report.json` dentro de `tmp/phase3/staging`, adem?s de lanzar la bater?a de seeds (`populate-organization-ids.seed.spec.ts`, `validate-organization-ids.seed.spec.ts`, `phase3-report.spec.ts`). Banderas ?tiles: `--summary-dir`, `--options-path`, `--report-path`, `--skip-run`, `--skip-tests`, `--jest-pattern`, `--summary-stdout`.
...
### 2024-06-07 – Robustez y parametrización avanzada de `phase3:report`

- **Contexto:** Tras habilitar el reporte consolidado fue necesario robustecer su CLI para escenarios mixtos (corridas manuales, pipelines y ejecuciones incrementales) donde las rutas provienen de variables de entorno y algunos resúmenes pueden no existir todavía.
- **Implementación:** Se expuso `parsePhase3ReportCliArgs`/`resolvePhase3ReportOptions` para combinar banderas y variables `PHASE3_REPORT_*`, normalizar rutas relativas/absolutas y permitir que `PHASE3_OPTIONS_PATH` actúe como alias de salida. El helper `readSummary` ahora ignora archivos ausentes (`ENOENT`) y la utilidad `parseEnvBoolean` valida banderas booleanas (`PHASE3_REPORT_STDOUT`). La suite [`backend/test/phase3-report.spec.ts`](../test/phase3-report.spec.ts) cubre estos escenarios comprobando la agregación del reporte, los _warnings_ y la resolución de rutas.
- **Resultado:** `npm run phase3:report` funciona con defaults sensibles (`tmp/phase3`), tolera resúmenes faltantes, respeta overrides de entorno/CLI y mantiene la exportación opcional del JSON consolidado sin generar falsas fallas en pipelines.
