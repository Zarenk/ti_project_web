...
export class AdsRbacGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    const requiredRoles = this.reflector.getAllAndOverride<AdsRole[]>(ROLES_KEY, [
      context.getHandler(),
      context.getClass(),
    ]) || [];
    const organizationScoped = this.reflector.getAllAndOverride<boolean>(ORGANIZATION_KEY, [
      context.getHandler(),
      context.getClass(),
    ]);
    ...
    if (organizationScoped) {
      const orgParam = request.params?.organizationId ?? request.body?.organizationId ?? request.query?.organizationId;
