...
function generateBaseXML(data: any, documentType: string) {
  const doc = create({ version: '1.0', encoding: 'UTF-8' })
    ...
    .ele('cbc:ID').txt(`${data.serie}-${data.correlativo}`).up()
    .ele('cbc:InvoiceTypeCode').txt(documentType === 'Invoice' ? '01' : (documentType === 'CreditNote' ? '07' : '03')).up()
    .ele('cbc:IssueDate').txt(data.fechaEmision).up()
    .ele('cbc:DocumentCurrencyCode').txt(data.tipoMoneda).up()
    .ele('cac:AccountingSupplierParty')
      .ele('cbc:CustomerAssignedAccountID').txt(data.emisor.ruc).up()
      .ele('cbc:AdditionalAccountID').txt('6').up()
      .ele('cac:Party')
        .ele('cac:PartyName')
          .ele('cbc:Name').txt(data.emisor.razonSocial).up()
        .up().up().up()
    .ele('cac:AccountingCustomerParty')
      .ele('cbc:CustomerAssignedAccountID').txt(data.cliente.ruc).up()
      .ele('cbc:AdditionalAccountID').txt('6').up()
      .ele('cac:Party')
        .ele('cac:PartyName')
          .ele('cbc:Name').txt(data.cliente.razonSocial).up()
        .up().up().up()
    .ele('cac:TaxTotal')
      .ele('cbc:TaxAmount', { currencyID: data.tipoMoneda }).txt('18.00').up()
    .up()
    .ele('cac:LegalMonetaryTotal')
      .ele('cbc:PayableAmount', { currencyID: data.tipoMoneda }).txt(Number(data.total || 0).toFixed(2)).up()
    .up();
...
export function generateInvoiceXML(data: any): string {
  ...
  for (const [index, item] of data.items.entries()) {
    doc.ele('cac:InvoiceLine')
      ...
      .ele('cbc:InvoicedQuantity', { unitCode: 'NIU' }).txt(String(item.cantidad)).up()
      .ele('cbc:LineExtensionAmount', { currencyID: data.tipoMoneda }).txt(Number(item.total || 0).toFixed(2)).up()
      .ele('cac:Item')
        .ele('cbc:Description').txt(item.descripcion).up()
      .up()
      .ele('cac:Price')
        .ele('cbc:PriceAmount', { currencyID: data.tipoMoneda }).txt(Number(item.precioUnitario || 0).toFixed(2)).up()
      .up()
      .ele('cac:TaxTotal')
        .ele('cbc:TaxAmount', { currencyID: data.tipoMoneda }).txt(Number(item.igv || 0).toFixed(2)).up()
        .ele('cac:TaxSubtotal')
          .ele('cbc:TaxableAmount', { currencyID: data.tipoMoneda }).txt(Number(item.subTotal || 0).toFixed(2)).up()
          .ele('cbc:TaxAmount', { currencyID: data.tipoMoneda }).txt(Number(item.igv || 0).toFixed(2)).up()
          .ele('cac:TaxCategory')
            .ele('cac:TaxScheme')
...
export function generateBoletaXML(data: any): string {
  const tipoDocumentoCliente = data.cliente?.tipoDocumento === 'CE' ? '4' :
                             data.cliente?.tipoDocumento === 'PAS' ? '7' :
                             '1'; // por defecto, DNI
  ...
  const doc = create({ version: '1.0', encoding: 'UTF-8' })
    ...
    .ele('cbc:CustomizationID').txt('2.0').up()
    .ele('cbc:ID').txt(`${data.serie}-${data.correlativo}`).up()
    .ele('cbc:InvoiceTypeCode').txt('03').up()
    .ele('cbc:IssueDate').txt(data.fechaEmision).up()
    .ele('cbc:DocumentCurrencyCode').txt(data.tipoMoneda).up()
    .ele('cac:AccountingSupplierParty')
      .ele('cbc:CustomerAssignedAccountID').txt(data.emisor.ruc).up()
      ...
      .ele('cac:Party')
        ...
        .up().up().up()
    .ele('cac:AccountingCustomerParty')
      .ele('cbc:CustomerAssignedAccountID').txt(String(numeroDocumentoCliente)).up()
      .ele('cbc:AdditionalAccountID').txt(tipoDocumentoCliente).up()
      .ele('cac:Party')
        .ele('cac:PartyName')
          .ele('cbc:Name').txt(razonSocialCliente).up()
        .up().up().up()
    .ele('cac:TaxTotal')
      .ele('cbc:TaxAmount', { currencyID: data.tipoMoneda }).txt('18.00').up()
    .up()
