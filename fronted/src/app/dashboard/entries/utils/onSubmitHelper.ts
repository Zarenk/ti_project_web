...
/**
 * Save or update a draft entry. Same payload building as handleFormSubmission
 * but calls createDraftEntry/updateDraftEntry instead of createEntry.
 */
export async function handleDraftSubmission({
  data,
  form,
  stores,
  providers,
  ...
  try {
    ...
    if (selectedProducts.length === 0) {
      toast.error("Debe agregar al menos un producto al borrador.");
      return false;
    }
    ...
    if (isNewInvoiceBoolean) {
      ...
      if (existing) {
        ...
        if (!createdProvider?.id) throw new Error("No se pudo crear el proveedor.");
        providerIdBoolean = createdProvider.id;
      }
    }
    ...
    const verifiedProducts = await verifyOrCreateProducts(
    ...
    const updatedProducts = selectedProducts.map((product: any) => {
      const verifiedProduct = verifiedProducts.find((vp: any) => vp.name === product.name);
      if (!verifiedProduct) throw new Error(`No se encontro un ID para el producto: ${product.name}`);
      const priceInSoles =
        tipoMoneda === "USD" && tipoCambioActual
          ? product.price * tipoCambioActual
          : product.price;
      return {
        productId: verifiedProduct.id,
    ...
    const payload = {
      ...
      tipoMoneda: data.tipo_moneda,
      paymentMethod: data.payment_method,
      paymentTerm: data.payment_method,
      details: updatedProducts,
      totalGross: totalFromInput,
      invoice: hasInvoice
        ? {
    ...
    if (existingDraftId) {
      await updateDraftEntry(existingDraftId, payload);
      toast.success("Borrador actualizado correctamente.");
    } else {
      await createDraftEntry(payload);
    ...
    toast.error(error.message || "Ocurrió un error al guardar el borrador.");
    return false;
  }
}
