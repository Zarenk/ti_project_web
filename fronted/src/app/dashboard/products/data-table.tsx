...
export function DataTable<TData extends {id:string, createdAt:Date, name:string,
  ...
  // Contar el número total de filas seleccionadas
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      await deleteProducts(selectedIds); // Llama a la API para eliminar los productos
              ...
              <tbody>
                ${selectedData
                  ...
                  <tr>
                    ...
                    <td class="truncate" title="${row.category_name}">
                      ${row.category_name}
                    </td>
  ...
  // MODAL DE EDICION MASIVO
    ...
    const handleEditSelected = () => {
      ...
      if (selectedIds.length === 1) {
        ...
        router.push(`/dashboard/products/${selectedId}/edit`); // Redirige al formulario de edición con el ID
      }
      ...
      <div className="py-2 px-4 flex flex-col sm:flex-row items-start sm:items-center space-x-4">
          ...
          {isEditModalOpen && (
            <AlertDialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>Editar Productos Seleccionados</AlertDialogTitle>
                  <AlertDialogDescription>
                    Realiza los cambios que deseas aplicar a los productos seleccionados.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Input
                      type="text"
                      defaultValue={localEditValuesRef.current?.price || ""}
                      maxLength={10} // Limita el número de caracteres (por ejemplo, 10)
                      onChange={(e) => {
                        const value = e.target.value;
                        // Validar que el valor sea un número con hasta 2 decimales
                        if (/^\d*\.?\d{0,2}$/.test(value)) {
                          handleEditChange("price", value); // Actualiza el valor en el estado
                        }
                      }}
                      onKeyDown={(e) => {
                      ...
                      onBlur={(e) => {
                        // Asegurarse de que el valor final tenga 2 decimales
                        const value = parseFloat(e.target.value);
                        if (!isNaN(value)) {
                          const roundedValue = value.toFixed(2);
                          handleEditChange("price", parseFloat(roundedValue));
                        }
                      }}
                  ...
                  <div>
                    ...
                    <Input
                      type="text"
                      maxLength={10} // Limita el número de caracteres (por ejemplo, 10)
                      defaultValue={localEditValuesRef.current?.priceSell || ""}
                      onChange={(e) => {
                        const value = e.target.value;
                        // Validar que el valor sea un número con hasta 2 decimales
                        if (/^\d*\.?\d{0,2}$/.test(value)) {
                          handleEditChange("priceSell", value); // Actualiza el valor en el estado
                        }
                      }}
                      onKeyDown={(e) => {
                        // Evitar que se ingresen letras o caracteres no deseados
                      ...
                      onBlur={(e) => {
                        ...
                        if (!isNaN(value)) {
                          const roundedValue = value.toFixed(2);
                          handleEditChange("priceSell", parseFloat(roundedValue));
                        }
                      }}
                      className="w-full border rounded-md px-2 py-1"
                    />
                  ...
                  {selectedRowsData.length > 0 ? (
                    selectedRowsData.map((row) => (
                      ...
                      >
                        ...
                        <div>
                          <strong>Precio de Venta:</strong> S/. {row.priceSell}
                        </div>
                        <div>
