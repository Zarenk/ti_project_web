...
export function DataTable<TData extends { createdAt: string; description: string; type: string }, TValue>({
  columns,
  data,
  ...
  const filteredData = useMemo(() => {
    ...
    if (activityType && activityType !== "ALL") {
      result = result.filter((item) => (item as any).type === activityType);
    }
    ...
    if (search) {
      ...
      result = result.filter((item) =>
        (item as any).description.toLowerCase().includes(s)
      );
    }
    return result;
  }, [data, search, selectedDateRange, activityType]);
  ...
  const isFiltered = !!search || !!selectedDateRange || activityType !== "ALL";
