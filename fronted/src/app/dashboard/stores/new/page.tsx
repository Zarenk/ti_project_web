...
export default async function StoresNewPage({params}: Props) {
    ...
    // resolvedParams.id existe. Si existe, llamamos a la función getStore para 
    // obtener los datos del store:
    const store = resolvedParams.id ? await getStore(resolvedParams.id) : null;
    ...
    <div className="flex justify-center items-start min-h-screen p-3">
        ...
        <Card className="w-full max-w-lg sm:max-w-md md:max-w-lg lg:max-w-2xl" >
            <CardHeader className="pb-2 sm:pb-2">
                ...
                stamos creando un store nuevo o actualizando uno existente. 
                Esto se refleja en el título del formulario:*/}
                <CardTitle className="text-center text-xl font-bold pt-5">
                    {resolvedParams.id ? 'Actualizar Tienda' : 'Crear Tienda'}
                </CardTitle>
            </CardHeader>
            <CardContent>
                <StoreForm store={store}></StoreForm>
            </CardContent>
        </Card>
