...
import { getProduct, getProducts, getPublicProduct, getPublicProducts } from "../../dashboard/products/products.api"
  ...
  const handleRelatedProductEditClick = useCallback(
    ...
    async (productId: number) => {
      ...
      try {
        ...
        const hasAuth = Boolean(await getAuthToken())
        const productData = hasAuth ? await getProduct(String(productId)) : await getPublicProduct(String(productId))
        ...
        const mapped = await Promise.all(
          ...
          filtered.slice(0, 4).map(async (p: any) => {
            ...
            try {
              ...
              const stores = hasAuth
              ? await getStoresWithProduct(p.id)
              : await getPublicStoresWithProduct(p.id)
