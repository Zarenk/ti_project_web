...
export default function VerifyDetailPage() {
      ...
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-sky-50 dark:from-slate-900 dark:via-slate-950 dark:to-slate-900">
        <div className="container mx-auto px-4 py-16 max-w-md flex flex-col items-center gap-4 animate-in fade-in-0 slide-in-from-bottom-4 duration-500">
          ...
          <p className="text-sm text-muted-foreground text-center">
            No se encontro un comprobante con el codigo proporcionado. Verifica
            que el enlace o codigo QR sea correcto.
          </p>
          <Link href="/verify">
  ...
  const isAccepted =
    data.sunat?.estado === "ACCEPTED"
  const isFailed =
    data.sunat?.estado === "FAILED" ||
    data.sunat?.estado === "ERROR" ||
    data.sunat?.estado === "REJECTED"

  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-sky-50 dark:from-slate-900 dark:via-slate-950 dark:to-slate-900">
      <div className="container mx-auto px-4 py-4 sm:py-8 max-w-2xl w-full min-w-0">
        {/* Top navigation */}
        <div className="mb-4 animate-in fade-in-0 duration-300">
          <Link href="/verify">
            <Button
              variant="ghost"
              size="sm"
              className="cursor-pointer gap-1.5 text-muted-foreground hover:text-foreground transition-colors"
            >
              <ArrowLeft className="h-4 w-4" />
              <span className="text-sm">Verificar otro comprobante</span>
            </Button>
          </Link>
        </div>
        ...
        <div className="flex flex-col gap-3 sm:gap-4 w-full min-w-0">
          {/* Emisor Card */}
          <Card
            className="w-full min-w-0 overflow-hidden animate-in fade-in-0 slide-in-from-bottom-4 duration-500"
          ...
          >
            ...
            <CardContent className="overflow-hidden px-4 sm:px-6 pb-4">
            ...
            <CardContent className="overflow-hidden px-4 sm:px-6 pb-4">
              ...
              <div className="grid grid-cols-2 gap-y-1 text-sm">
                ...
                <span className="text-right">
                  {formatDate(data.comprobante.fechaEmision)}
                </span>
              ...
              <div className="space-y-1.5 text-sm">
                ...
                {data.montos.exonerada > 0 && (
                  <div className="flex justify-between">
                    ...
                    <span>
                      {formatCurrency(data.montos.exonerada, currency)}
                    </span>
                  </div>
                )}
                {data.montos.inafecta > 0 && (
                  <div className="flex justify-between">
                    ...
                    <span>
                      {formatCurrency(data.montos.inafecta, currency)}
                    </span>
                  </div>
                )}
                <Separator />
              ...
              <CardContent className="overflow-hidden px-4 sm:px-6 pb-4">
                ...
                {data.sunat.fecha && (
                  <p className="text-xs text-muted-foreground mt-1">
                    Enviado: {formatDateTime(data.sunat.fecha)}
                  </p>
                )}
              </CardContent>
            </Card>
          ...
          {/* Actions */}
          <div
            className="flex flex-col sm:flex-row gap-3 mt-2 pb-6 animate-in fade-in-0 slide-in-from-bottom-4 duration-500"
