  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        const [ordersById, ordersByEmail, ordersByDni] = await Promise.all([
          getOrdersByUser(profile.id).catch(() => []),
          profile.email ? getOrdersByEmail(profile.email).catch(() => []) : Promise.resolve([]),
          client?.typeNumber ? getOrdersByDni(client.typeNumber).catch(() => []) : Promise.resolve([]),
        ])
        const byId = Array.isArray(ordersById) ? ordersById : []
        const byEmail = Array.isArray(ordersByEmail) ? ordersByEmail : []
        const byDni = Array.isArray(ordersByDni) ? ordersByDni : []
        const merged = [...byId, ...byEmail, ...byDni].reduce((acc: Map<number, any>, curr: any) => {
          if (!acc.has(curr.id)) acc.set(curr.id, curr)
          return acc
        }, new Map()).values()
        const historyOrders = Array.from(merged).map((o: any) => {
        ...
        // Ordenar por fecha descendente ya que se mezclan dos fuentes
        setOrderHistory(historyOrders.sort((a: any, b: any) => b.date.getTime() - a.date.getTime()))
